...
export async function cancerSpecificCoExpression(req: Request, res: Response): Promise<any> {
    setHeaders(res);
    ...
    if (queryMap.target) {
        ...
        try {
            let dataDescriptions: any = await PairwiseService.getAllDataDescs('TCGA', 'Gene_Coexpression');
            let dataDescIds = [];
            ...
            try {
                await PairwiseService.searchTermSecondaryPathways(
                    ...
                    .then(data => {
                        const ps = new PredictionSet("Cancer Specific Coexpression", "Protein", "Cancer Type",
                        ...
                        Object.entries(data).forEach((match: any) => {
                            {
                                ...
                                match.posGenes.forEach((gene: any) => {
                                    ...
                                    ps.addPrediction(match.gene, "", cancerType, extraFields);
                                })
                            }
                        })
...
exports.cancerSpecificCoExpression = cancerSpecificCoExpression;